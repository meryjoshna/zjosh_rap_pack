CLASS lsc_ZJOSH_I_TRAVEL_UN DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZJOSH_I_TRAVEL_UN IMPLEMENTATION.
  ...
  METHOD adjust_numbers.
    ...
    mapped-zjosh_i_travel_un = VALUE #( FOR ls_travel IN lt_travel_mapping (
                                        %tmp = VALUE #( travelid = ls_travel-preliminary-travel_id )
                                        TravelID = ls_travel-final-travel_id  ) ).

    mapped-zjosh_i_bookg_u = VALUE #( FOR ls_booking IN lt_booking_mapping (
